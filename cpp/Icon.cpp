...
QIcon load_icon_from_uri(QString const& uri)
{
    return QIcon { uri };
}
...
{
    return {
        static_cast<int>(std::ceil(size.width() * device_pixel_ratio)),
        static_cast<int>(std::ceil(size.height() * device_pixel_ratio)),
    };
}
